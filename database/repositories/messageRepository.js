...
class MessageRepository {
  ...
  async getEnviosCobradosTodos(data_inicio, data_fim) {
    ...
    const query =
      "SELECT c.idcliente AS cnpj, c.nome AS nome_cliente,  COUNT(*) AS total FROM envioscobrados AS e INNER JOIN cliente AS c ON CAST(e.idcliente AS integer) = c.id WHERE e.datainclusao BETWEEN $1 AND $2 GROUP BY e.idcliente, c.nome, c.idcliente ORDER BY nome_cliente ASC";
    const dados = await new Promise((resolve, reject) => {
