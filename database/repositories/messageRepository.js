...
class MessageRepository {
  ...
  async getAllTokens(dadosMensagens, idCliente) {
    const query =
      'SELECT id, chave, idcliente, data_atendimento, hora_atendimento, nomepaciente, id_local, medico FROM "enviostoken" WHERE "id" > $1 AND "idcliente" = $2 AND token IS NOT null ORDER BY id ASC';
    const dados = await new Promise((resolve, reject) => {
      db.query(query, [dadosMensagens.id, idCliente], (erro, result) => {
        if (erro) {
          console.log(erro);
